...
const obj = {
	id: uuidv1(), // 主键id
	school_id: 'school_id_value', // 外键，学校id，关联学校表
	state: 5, // 激活状态，0：已激活，1：待激活
};
...
export const add = async () => {
	it('should HTTP /add', async () => {
		const q1 = `mutation{
			addSchoolactivation(schoolactivation:${ spread(obj) }){
				id, school_id, state
			}
		}`;
...
export const update = async () => {
	it('should HTTP /update', async () => {
		const q1 = `mutation{
			updateSchoolactivation(schoolactivation:${ spread(obj) }){
				err, msg, desc, info
			}
		}`;
