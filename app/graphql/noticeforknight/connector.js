...
class NoticeforknightConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Noticeforknight;
	}
	...
	// 新增模型的方法
	async addNoticeforknight(noticeforknight) {
		const {ctx} = this;
		// 开启一个事务
		// const transaction = await ctx.model.transaction();
		try{
			const {id,partner_id,ntype,title,content,created} = noticeforknight;
			id = uuidv1();
			console.log('【add noticeforknight】 ： ', noticeforknight);
			const noticeforknightInput = await this.proxy.create( _.omitBy({id,partner_id,ntype,title,content,created}, _.isNil) );
			return noticeforknightInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add noticeforknight】： ', msg);
	...
	// 修改模型的方法
	async updateNoticeforknight(noticeforknight) {
		const {ctx} = this;
		const {id,partner_id,ntype,title,content,created} = noticeforknight;
		let msg = null, result = -1;
		try{
			result = await this.proxy.update( _.omitBy({id,partner_id,ntype,title,content,created}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update noticeforknight】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	// 删除模型的方法
	async removeNoticeforknight(id) {
		const {ctx} = this;
		let msg = null, result = -1;
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete noticeforknight】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
...
module.exports = NoticeforknightConnector;
