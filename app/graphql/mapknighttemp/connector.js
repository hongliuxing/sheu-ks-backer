...
class MapknighttempConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Mapknighttemp;
	}
	...
	// 新增模型的方法
	async addMapknighttemp(mapknighttemp) {
		const {ctx} = this;
		// 开启一个事务
		// const transaction = await ctx.model.transaction();
		try{
			const {id,knight_id,latitude,longitude,created,modify_time} = mapknighttemp;
			id = uuidv1();
			console.log('【add mapknighttemp】 ： ', mapknighttemp);
			const mapknighttempInput = await this.proxy.create( _.omitBy({id,knight_id,latitude,longitude,created,modify_time}, _.isNil) );
			return mapknighttempInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add mapknighttemp】： ', msg);
	...
	// 修改模型的方法
	async updateMapknighttemp(mapknighttemp) {
		const {ctx} = this;
		const {id,knight_id,latitude,longitude,created,modify_time} = mapknighttemp;
		let msg = null, result = -1;
		try{
			result = await this.proxy.update( _.omitBy({id,knight_id,latitude,longitude,created,modify_time}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update mapknighttemp】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeMapknighttemp(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete mapknighttemp】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
...
module.exports = MapknighttempConnector;
