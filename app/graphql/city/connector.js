...
class CityConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.City;
	}
	...
	async addCity(city) {
		...
		try{
			const {id,code,name,provincecode} = city;
			id = uuidv1();
			console.log('【add city】 ： ', city);
			const cityInput = await this.proxy.create( _.omitBy({id,code,name,provincecode}, _.isNil) );
			return cityInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add city】： ', msg);
	...
	async updateCity(city) {
		...
		try{
			result = await this.proxy.update( _.omitBy({id,code,name,provincecode}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update city】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeCity(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete city】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
