...
class CommentConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
	...
	async addComment(comment) {
		...
		try{
			const {id,seller_id,author_id,order_id,anonymity,score,content,struts,created} = comment;
			id = uuidv1();
			console.log('【add comment】 ： ', comment);
			const commentInput = await this.proxy.create( _.omitBy({id,seller_id,author_id,order_id,anonymity,score,content,struts,created}, _.isNil) );
			return commentInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add comment】： ', msg);
	...
	async updateComment(comment) {
		...
		try{
			result = await this.proxy.update( _.omitBy({id,seller_id,author_id,order_id,anonymity,score,content,struts,created}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update comment】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeComment(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete comment】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
