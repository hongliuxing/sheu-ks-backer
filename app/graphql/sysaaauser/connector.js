...
class SysaaauserConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Sysaaauser;
	}
	...
	// 新增模型的方法
	async addSysaaauser(sysaaauser) {
		const {ctx} = this;
		// 开启一个事务
		// const transaction = await ctx.model.transaction();
		try{
			const {id,unionid,openid,nickname,realname,telephone,logo_url,gender,struts,created} = sysaaauser;
			id = uuidv1();
			console.log('【add sysaaauser】 ： ', sysaaauser);
			const sysaaauserInput = await this.proxy.create( _.omitBy({id,unionid,openid,nickname,realname,telephone,logo_url,gender,struts,created}, _.isNil) );
			return sysaaauserInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add sysaaauser】： ', msg);
	...
	// 修改模型的方法
	async updateSysaaauser(sysaaauser) {
		const {ctx} = this;
		const {id,unionid,openid,nickname,realname,telephone,logo_url,gender,struts,created} = sysaaauser;
		let msg = null, result = -1;
		try{
			result = await this.proxy.update( _.omitBy({id,unionid,openid,nickname,realname,telephone,logo_url,gender,struts,created}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update sysaaauser】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeSysaaauser(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete sysaaauser】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
...
module.exports = SysaaauserConnector;
