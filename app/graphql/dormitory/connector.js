...
class DormitoryConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Dormitory;
	}
	...
	// 新增模型的方法
	async addDormitory(dormitory) {
		const {ctx} = this;
		// 开启一个事务
		// const transaction = await ctx.model.transaction();
		try{
			const {id,drom_name,drom_num,school_id} = dormitory;
			id = uuidv1();
			console.log('【add dormitory】 ： ', dormitory);
			const dormitoryInput = await this.proxy.create( _.omitBy({id,drom_name,drom_num,school_id}, _.isNil) );
			return dormitoryInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add dormitory】： ', msg);
	...
	async updateDormitory(dormitory) {
		...
		try{
			result = await this.proxy.update( _.omitBy({id,drom_name,drom_num,school_id}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update dormitory】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeDormitory(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete dormitory】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
...
module.exports = DormitoryConnector;
