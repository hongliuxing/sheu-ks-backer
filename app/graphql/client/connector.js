...
class ClientConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Client;
	}
	...
	async addClient(client) {
		...
		try{
			const {id,unionid,openid,nickname,realname,telephone,logo_url,gender,struts,created,dorm_id} = client;
			id = uuidv1();
			console.log('【add client】 ： ', client);
			const clientInput = await this.proxy.create( _.omitBy({id,unionid,openid,nickname,realname,telephone
				,logo_url,gender,struts,created,dorm_id}, _.isNil) );
			return clientInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add client】： ', msg);
	...
	async updateClient(client) {
		...
		try{
			result = await this.proxy.update( _.omitBy({id,unionid,openid,nickname,realname,telephone,logo_url
				,gender,struts,created,dorm_id}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update client】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeClient(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete client】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
