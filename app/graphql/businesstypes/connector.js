...
class BusinesstypesConnector extends BaseConnector {
	constructor(ctx) {
		super(ctx);
		this.proxy = ctx.app.model.Businesstypes;
	}
	...
	// 新增模型的方法
	async addBusinesstypes(businesstypes) {
		const {ctx} = this;
		// 开启一个事务
		// const transaction = await ctx.model.transaction();
		try{
			const {id,btype,legal_person,telephone,license_url,struts,created} = businesstypes;
			id = uuidv1();
			console.log('【add businesstypes】 ： ', businesstypes);
			const businesstypesInput = await this.proxy.create( _.omitBy({id,btype,legal_person,telephone,license_url,struts,created}, _.isNil) );
			return businesstypesInput;
		}catch(err) {
			const msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR add businesstypes】： ', msg);
	...
	// 修改模型的方法
	async updateBusinesstypes(businesstypes) {
		const {ctx} = this;
		const {id,btype,legal_person,telephone,license_url,struts,created} = businesstypes;
		let msg = null, result = -1;
		try{
			result = await this.proxy.update( _.omitBy({id,btype,legal_person,telephone,license_url,struts,created}, _.isNil) , { where: {id} });
			...
			console.error('【ERROR update businesstypes】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
	...
	async removeBusinesstypes(id) {
		...
		try{
			...
			msg = new Message({err: ErrorType.DATABASE_ERROR, desc: err});
			console.error('【ERROR delete businesstypes】： ', msg);
			ctx.logger.error(msg);
		}
		return msg;
	}
...
module.exports = BusinesstypesConnector;
